...
test.group('Feature: Get rider trip history', (group) => {
  ...
  group.each.setup(async () => {
    await clearDatabase()
    await PgSqlTrip.createMany([otherTrip, riderTrip1, riderTrip2])
  })

  test('gets rider trip history', async ({ client, assert }) => {
    const response = await client.get('/trips/history')
    response.assertStatus(200)
    response.assertBody({
      riderId,
      trips: response.body().trips,
    })
    assert.sameDeepMembers(response.body().trips, [
      {
