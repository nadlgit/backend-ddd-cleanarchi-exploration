...
export class BookTripUseCase {
  ...
  async execute({
    ...
    this.assertRiderCanBookTrip(rider, hasRiderTripInProgress, riderTodayTripCount)

    const { startZone, endZone, distanceKm } = await this.tripScannerGateway.retrieveTripRouteInfo(
      startLocation,
      endLocation
    )
  ...
  private async retrieveRiderData(riderId: string) {
    const [rider, riderTripCountByStatus, riderTodayTripCount] = await this.unitOfWork.execute(
    ...
    return {
      ...
      hasRiderTripInProgress:
        !!riderTripCountByStatus['BOOKED'] || !!riderTripCountByStatus['CONFIRMED'],
      riderTodayTripCount,
    }
  }

  private assertRiderCanBookTrip(rider: Rider, hasTripInProgress: boolean, dayTripCount: number) {
