import { NoDriverAvailable } from '#drivers/core/events/no-driver-available'
import { type LocationDistanceGateway } from '#drivers/core/ports/providers/location-distance-gateway'
import { type DriverUnitOfWork } from '#drivers/core/ports/repositories/driver-unit-of-work'
...
export class MatchDriverUseCase {
  ...
  async execute({ tripId, startLocation, carCategory }: MatchDriverCommand) {
    ...
    await this.unitOfWork.executeInTransaction(async ({ driverRepository, domainEventOutbox }) => {
  ...
  private async findAvailableDriver(tripStartLocation: string, carCategory: CarCategory) {
    const availableDrivers = await this.unitOfWork.execute(async ({ driverRepository }) =>
